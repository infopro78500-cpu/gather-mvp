...
- **Maîtrise des coûts Supabase** (7 juillet 2026) : trois optimisations pour que l'offre gratuite/Pro ne dérape pas en coût. **(1) Purge automatique** — l'expiration d'un événement ne supprimait pas les fichiers (le stockage montait indéfiniment) ; un cron quotidien (`/api/cron/cleanup-expired`, service role, protégé par `CRON_SECRET`, `vercel.json`) supprime les photos des événements expirés. **(2) Fin du re-téléchargement toutes les 8 s** — le rafraîchissement auto régénérait les URLs signées à chaque cycle, cassant le cache navigateur ; désormais les URLs sont mises en cache et réutilisées (URL stable → image en cache). **(3) Vignettes redimensionnées** — la grille peut demander des images redimensionnées (transformation Supabase) via `NEXT_PUBLIC_IMAGE_THUMB_WIDTH`, désactivé par défaut (transform = fonctionnalité payante), à activer sur l'offre Pro (~15× moins de bande passante sur la galerie). **Dimensionnement freemium conseillé** : jouer sur la rétention + le plafond de photos par événement (gratuit = courte durée + purge réelle), pas sur les Go bruts. **À décider par le porteur** : le cron purgera les 30 événements déjà expirés (dont les données restaurées « Nowel à Marcourt ») à sa première exécution — préserver certains comme démo si besoin.
